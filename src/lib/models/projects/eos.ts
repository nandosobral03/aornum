...
export const eos: ProjectDetails = {
    name: "Eos",
    links: [
        {
    ...
    description: `
        Eos is a starterpage that combines the things I use the most when I launch a new tab with extra
        functionality including notetaking, RSS feed aggregation, spotify tracking stats and more. The
        project was inspired by ricing communities like <a href="https://www.reddit.com/r/startpages/" target="_blank">r/startpages</a> and
        <a href="https://www.reddit.com/r/unixporn/" target="_blank">r/unixporn</a> as well as other <i> ricing </i> communities.

        I wanted to create a page that both looked good and was
        functional. The main goal of the project was to create a page that I would use everyday and it
        has become just that. I wanted it to be pretty customizable so I created a settings section that
        allows you to change how it looks. From changing the color scheme, saving and loading themes, to
        changing the background image. You can also select what tabs to show on the upper-right modal
        which include bookmarks, notes, spotify, and an episode tracker. In addition to that you can add
        RSS feeds to the page and it will display the latest articles from those feeds, modify the
        bookmarks, connect to the spotify API, select a time frame for the stats and show a currently
        playing song. Finally you can change the bottom right modal to either show a static image, the
        NASA image of the day nothing at all. The notes are saved as Markdown files in the database and
        can be edited and deleted from the app, showing a preview of the markdown as you type.

        The project is technically live and hosted on <a href="http://eos.aornum.xyz/" target="_blank">eos.aornum.xyz</a> 
        but it's not really meant to be used by anyone else. It's more of a personal project that I use everyday. So it is behind 
        a login screen and you can't create an account. I might change that in the future but for now it's just for me.
    `,
    technologies: `
