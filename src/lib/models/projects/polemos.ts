...
export const polemos: ProjectDetails = {
    name: "Polemos",
    links: [
        {
    ...
    description: `
        Polemos is a text based battle simulator. With a group of friends we used to play  <a href="https://brantsteele.net/hungergames/"> BrantSteele's Hunger Games Simulator</a> with our own custom characters,
        each of us representing a couple of teams. But as we did we found we wanted to play with more than the characters the simulator allowed us to, so I decided to make my own version of it, giving us the 
        ability to add as many characters as we wanted, as well as our own custom events, scenarios, statuses and add ourselves as sponsors of each of the teams. 
        <br><br>
        We also wanted to keep track of the results of each of the games, so I added overall stats for each of the characters, keeping track of those with most wins, kills, deaths, etc.
        The project includes the create teams, players, events, scenarios, statuses and sponsors. As well as the ability to run a simulation with a custom number of teams and players.
        As the simulation runs you can see the events that are happening and the status of each of the players, this creates a "Story" that is shown day by day and is saved on the server so it can be 
        viewed later.

        The project is live and hosted on <a href="http://polemos.aornum.xyz/" target="_blank">polemos.aornum.xyz</a> and can be accessed by anyone on
        guest mode, where all data is removed and replaced with defaults every 24 hours.
        `,
    technologies: `
