<script lang="ts">
	...
	const handleScroll = (e: Event) => {
		...
		let section = ranges.find(
			(range) => scrollTop >= range.offset - 5 && scrollTop <= range.offset + range.height + 5
		);
		if (section) {
		...
		<nav>
			{#each ranges as section, i}
				<div
					...
					on:keydown={(e) =>
						e.key === 'Enter' &&
						scrollContainer.scrollTo({ top: section.offset, behavior: 'smooth' })}
				>
